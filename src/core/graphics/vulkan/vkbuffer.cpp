...
UniformBufferManager::Vulkan::~Vulkan() noexcept {
...
bool UniformBufferManager::Vulkan::create(UniformBufferType type, size_t size_in_bytes) {
...
bool UniformBufferManager::Vulkan::update(UniformBufferType type, uint32_t frame_index, const void *data, size_t size_in_bytes, size_t offset_in_bytes) const {
...
VkDescriptorBufferInfo UniformBufferManager::Vulkan::getDescriptorBufferInfo(UniformBufferType type, uint32_t frame_index) const {
...
bool UniformBufferManager::Vulkan::createBufferForFrame(BufferFrame &bframe, uint32_t size) const {
  GraphicsAPI::Vulkan::DeviceManager &device_manager = vulkan->getDeviceManager();
  bframe.size = size;
