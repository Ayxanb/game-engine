...
#include <core/graphics/opengl/glbuffer.hpp>

namespace Engine {

bool UniformBufferManager::OpenGL::create(UniformBufferType type, size_t size_in_bytes) {
...
bool UniformBufferManager::OpenGL::update(UniformBufferType type, uint32_t, const void * data, 
...
void UniformBufferManager::OpenGL::bind(UniformBufferType type, uint32_t binding_point) const {
  auto it = uniform_buffers.find(type);
  if (it == uniform_buffers.end()) {
    LOG_ERROR("[GraphicsAPI::OpenGL]: Uniform buffer type `{}` not found for binding", uniformBufferTypeToString(type));
    return;
  }
